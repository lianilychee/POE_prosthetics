...
//
// June 2012: first version
// July 2013: The 'int' in the union for the x,y,z changed into int16_t to be compatible with Arduino Due.
//
// Open Source / Public Domain
//
// Using Arduino 1.0.1 It will not work with an older version, since Wire.endTransmission() uses a parameter 
...
// Register names according to the datasheet. According to the InvenSense document "MPU-6000 and MPU-6050 Register Map 
// and Descriptions Revision 3.2", there are no registers at 0x02 ... 0x18, but according other information 
// the registers in that unknown area are for gain and offsets.
...
// By using the bit number, programming the sensor is like programming the AVR microcontroller.
// But instead of using "(1<<X)", or "_BV(X)", the Arduino "bit(X)" is used.
#define MPU6050_D0 0
#define MPU6050_D1 1
#define MPU6050_D2 2
#define MPU6050_D3 3
...
// These are the names for the bits. Use these only with the bit() macro.
...
// Alternative names for the combined definitions
// This name uses the bandwidth (Hz) for the accelometer, for the gyro the bandwidth is almost the same.
...
// The XG_ST, YG_ST, ZG_ST are bits for selftest. The FS_SEL sets the range for the gyro.
// These are the names for the bits. Use these only with the bit() macro.
...
// Alternative names for the combined definitions. The name uses the range in degrees per second.
...
// The XA_ST, YA_ST, ZA_ST are bits for selftest. The AFS_SEL sets the range for the accelerometer.
// These are the names for the bits. Use these only with the bit() macro.
...
// Alternative names for the combined definitions. The name uses the Cut-off frequency.
